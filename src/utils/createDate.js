...
module.exports = (userTimeZone = serverTimeZone) => {
  ...
  const instance = {
    ...
    lastMilliseconds: (milliseconds) =>
      subMilliseconds(currentDate, milliseconds),
    ...
    includeDays: (days) =>
      subHours(subDays(startOfDay(currentDate), days - 1), timeZoneToleranz),
    includeMonths: (months) =>
      subHours(
        subMonths(startOfMonth(currentDate), months - 1),
        timeZoneToleranz,
      ),
    includeYears: (years) =>
      subHours(subYears(startOfYear(currentDate), years - 1), timeZoneToleranz),
  };

  // Get the last-function that matches the interval
  const lastFnByInterval = (interval) => {
    switch (interval) {
      case intervals.INTERVALS_DAILY:
        return instance.lastDays;
      case intervals.INTERVALS_MONTHLY:
        return instance.lastMonths;
      case intervals.INTERVALS_YEARLY:
