...
module.exports = {
  AverageViews: {
    ...
    change: pipe(requireAuth, (entries) => {
      const totalCountCurrent = entries
        .slice(1, 8)
        .reduce((acc, entry) => acc + entry.count, 0);
      const totalCountPrevious = entries
        .slice(8, 15)
        .reduce((acc, entry) => acc + entry.count, 0);
      ...
      return Math.min(
        Math.max(
          Math.round((totalDifference / totalCountPrevious) * 100),
          -100,
        ),
        100,
      );
