...
const RouteSettings = (props) => {
  ...
  const showDomainEditModal = (domain) =>
    props.addModal(MODALS_DOMAIN_EDIT, domain);
  const showEventAddModal = () => props.addModal(MODALS_EVENT_ADD);
  const showEventEditModal = (event) =>
    props.addModal(MODALS_EVENT_EDIT, event);
  const showPermanentTokenAddModal = () =>
    props.addModal(MODALS_PERMANENT_TOKEN_ADD);
  const showPermanentTokenEditModal = (permanentToken) =>
    props.addModal(MODALS_PERMANENT_TOKEN_EDIT, permanentToken);

  const domainsLoading = h(LoadingMessage, { label: "domains" });
  const eventsLoading = h(LoadingMessage, { label: "events" });
  const permanentTokensLoading = h(LoadingMessage, {
    label: "permanent tokens",
  });

  const domainsItems = createItems(
    domains.value,
    showDomainEditModal,
    showDomainAddModal,
    "New domain",
  );
  const eventsItems = createItems(
    events.value,
    showEventEditModal,
    showEventAddModal,
    "New event",
  );
  const permanentTokensItems = createItems(
    permanentTokens.value,
    showPermanentTokenEditModal,
    showPermanentTokenAddModal,
    "New permanent token",
  );
  ...
  return h(
    ...
    h(
      ...
      ...(domains.status.isInitializing === true
        ? [domainsLoading]
        : domainsItems),
    ),
    ...
    h(
      ...
      ...(events.status.isInitializing === true
        ? [eventsLoading]
        : eventsItems),
    ),
    ...
    h(
      ...
      ...(permanentTokens.status.isInitializing === true
        ? [permanentTokensLoading]
        : permanentTokensItems),
    ),
    ...
    h(
      ...
      h(
        LinkItem,
        { type: "a", href: homepage, target: "_blank", rel: "noopener" },
        "Ackee on GitHub",
      ),
      h(Line),
      h(
