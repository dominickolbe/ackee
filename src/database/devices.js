...
const get = async (ids, sorting, type, range, limit, dateDetails) => {
  const aggregation = (() => {
    if (type === constants.DEVICES_TYPE_NO_MODEL) {
      ...
      if (sorting === sortings.SORTINGS_NEW)
        return aggregateNewRecords(ids, ["deviceManufacturer"], limit);
      if (sorting === sortings.SORTINGS_RECENT)
        return aggregateRecentRecords(ids, ["deviceManufacturer"], limit);
    }
  ...
  const enhanceId = (id) => {
    if (type === constants.DEVICES_TYPE_NO_MODEL)
      return `${id.deviceManufacturer}`;
    if (type === constants.DEVICES_TYPE_WITH_MODEL)
      return `${id.deviceManufacturer} ${id.deviceName}`;
  };
