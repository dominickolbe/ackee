...
const get = async (ids, type, interval, limit, dateDetails) => {
  const aggregation = (() => {
    if (type === constants.VIEWS_TYPE_UNIQUE)
      return aggregateViews(ids, true, interval, limit, dateDetails);
    if (type === constants.VIEWS_TYPE_TOTAL)
      return aggregateViews(ids, false, interval, limit, dateDetails);
  })();
