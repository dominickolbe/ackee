...
const getChart = async (ids, type, interval, limit, dateDetails) => {
  const aggregation = (() => {
    if (type === "TOTAL")
      return aggregateActions(ids, false, interval, limit, dateDetails);
    if (type === "AVERAGE")
      return aggregateActions(ids, true, interval, limit, dateDetails);
  })();
  ...
  const aggregation = (() => {
    if (type === "TOTAL") {
      if (sorting === sortings.SORTINGS_TOP)
        return aggregateTopActions(ids, false, range, limit, dateDetails);
      if (sorting === sortings.SORTINGS_NEW)
        return aggregateNewActions(ids, limit);
      if (sorting === sortings.SORTINGS_RECENT)
        return aggregateRecentActions(ids, limit);
    }
    if (type === "AVERAGE") {
      if (sorting === sortings.SORTINGS_TOP)
        return aggregateTopActions(ids, true, range, limit, dateDetails);
      if (sorting === sortings.SORTINGS_NEW)
        return aggregateNewActions(ids, limit);
      if (sorting === sortings.SORTINGS_RECENT)
        return aggregateRecentActions(ids, limit);
