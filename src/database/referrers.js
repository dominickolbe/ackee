...
const get = async (ids, sorting, type, range, limit, dateDetails) => {
  const aggregation = (() => {
    ...
    if (type === constants.REFERRERS_TYPE_NO_SOURCE) {
      ...
      if (sorting === sortings.SORTINGS_NEW)
        return aggregateNewRecords(ids, ["siteReferrer"], limit);
      if (sorting === sortings.SORTINGS_RECENT)
        return aggregateRecentRecords(ids, ["siteReferrer"], limit);
    }
    if (type === constants.REFERRERS_TYPE_ONLY_SOURCE) {
      if (sorting === sortings.SORTINGS_TOP)
        return aggregateTopRecords(ids, ["source"], range, limit, dateDetails);
      if (sorting === sortings.SORTINGS_NEW)
        return aggregateNewRecords(ids, ["source"], limit);
      if (sorting === sortings.SORTINGS_RECENT)
        return aggregateRecentRecords(ids, ["source"], limit);
    }
  })();
