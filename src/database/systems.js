  ...
  const aggregation = (() => {
    if (type === constants.SYSTEMS_TYPE_NO_VERSION) {
      if (sorting === sortings.SORTINGS_TOP)
        return aggregateTopRecords(ids, ["osName"], range, limit, dateDetails);
      if (sorting === sortings.SORTINGS_NEW)
        return aggregateNewRecords(ids, ["osName"], limit);
      if (sorting === sortings.SORTINGS_RECENT)
        return aggregateRecentRecords(ids, ["osName"], limit);
    }
    if (type === constants.SYSTEMS_TYPE_WITH_VERSION) {
      ...
      if (sorting === sortings.SORTINGS_NEW)
        return aggregateNewRecords(ids, ["osName", "osVersion"], limit);
      if (sorting === sortings.SORTINGS_RECENT)
        return aggregateRecentRecords(ids, ["osName", "osVersion"], limit);
    }
  })();
