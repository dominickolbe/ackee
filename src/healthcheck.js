...
const checkServer = async (url) => {
  ...
  if (response.ok === false) {
    throw new Error(
      `Server is unhealthy and returned with the status '${response.status}'`,
    );
  }
};
...
const checkApi = async (url) => {
  ...
  if (response.ok === false) {
    throw new Error(
      `API is unhealthy and returned with the status '${response.status}'`,
    );
  }
};
...
const check = () =>
  Promise.all([
    ...
    checkApi(
      `http://localhost:${config.port}/.well-known/apollo/server-health`,
    ),
  ]);
